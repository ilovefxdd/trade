    ...
    public partial class FormTrade : Form
    {
        ...
        private void FormTrade_Load(object sender, EventArgs e)
        {
            ...
            if (ul.ShowDialog() == DialogResult.OK)
            {
                ...
                //    mdApi = new MdApi("8703980", "061513", "7070", "tcp://116.236.239.136:41213");
        ...
        public class Instrument
        {
            ...
            #endregion
            private string k_path = "c:\\wm\\1m\\";
            private string t_path = "c:\\wm\\tick\\";
            string kext = ".min";
            string text = ".tk";
            private int 顶底状态;
            ...
            private string id;
            public List<CThostFtdcDepthMarketDataField> tick;
            public List<CThostFtdcDepthMarketDataField> kmak;
            private BarSeries bs;
            private BarSeries tmpbs;
            private List<笔> bl;
            public FileStream kfs;
            public BinaryWriter kbw;
            public FileStream tfs;
        ...
        string[] idlist = new string[] { "ru1709", "CF709", "i1709", "jm1709", "rb1710", "RM709", "SR709", "l1709", "bu1709" };
        ...
        void subAllInstruments()
        {
            ...
            for (int i = 0; i < this.dtInstruments.Rows.Count; i++)
            {
                ...
                if (instrument == "ru1709" | instrument == "i1709" | instrument == "rb1710" | instrument == "RM709" | instrument == "bu1709" | instrument == "SR709" | instrument == "l1709" | instrument == "jm1709" | instrument == "CF709" | instrument == "i1709")
                {
                ...
                try
                {
                    ...
                    for (int i = 0; i < ilist.Count(); i++)
                    {
                        if (pDepthMarketData.InstrumentID == ilist[i].ID)
                        {
                            ...
                            //   w_tra(pDepthMarketData, pre_tick);
                           //   this.pre_tick = pDepthMarketData;
                            break;
                        }
                           this.BeginInvoke(new Action<EnumProgessState, string>(progress), EnumProgessState.OnError, "tick_count" + listMarketDatas.Count); 
                    }
        ...
        void tradeApi_OnRspQryInstrument(ref CThostFtdcInstrumentField pInstrument, ref CThostFtdcRspInfoField pRspInfo, int nRequestID, bool bIsLast)
        {
            if (pRspInfo.ErrorID == 0)
            {
              ...
              , pInstrument.LongMarginRatio, pInstrument.ShortMarginRatio, 0, pInstrument.MaxLimitOrderVolume, pInstrument.MinMarketOrderVolume, false);
                ...
                if (pInstrument.InstrumentID == "ru1609" | pInstrument.InstrumentID == "jm1609" | pInstrument.InstrumentID == "rb1610"
                       | pInstrument.InstrumentID == "RM609" | pInstrument.InstrumentID == "CF609" | pInstrument.InstrumentID == "SR609" | pInstrument.InstrumentID == "l1609"
                       | pInstrument.InstrumentID == "bu1609" | pInstrument.InstrumentID == "i1609")
                {
        ...
        {
            int x, y, h, w;
            h = tabPage2.Height - 20;
            w = tabPage2.Width - 20;

            Graphics graphics = tabPage2.CreateGraphics();//声明一个Graphics对象  
            Pen myPen = new Pen(Color.Blue, 1);//实例化Pen类
            Rectangle rect = new Rectangle(10, 10, w, h);
                ...
                if (id == ol[k].id)
                {
                    ...
                    if (counts > 0)
                    {
                        ...
                        for (int j = 0; j < counts; j++)
                        {
                            if (j == 0)
                            {
                                hi = ol[k].tick[counts - 1].LastPrice;
                                lo = ol[k].tick[counts - 1].LastPrice;
                            }
                            if (j < 600 && j > 0)
                            {
                                if (ol[k].tick[counts - 1 - j].LastPrice > hi)
                                    hi = ol[k].tick[counts - 1 - j].LastPrice;
                                if (ol[k].tick[counts - 1 - j].LastPrice < lo)
                                    lo = ol[k].tick[counts - 1 - j].LastPrice;
                            }
                        }
                        ...
                        for (int i = 0; i < counts; i++)
                        {
                            if (i < 600)
                            {
                                rh = ((int)ol[k].tick[counts - i - 1].LastPrice - (int)lo);
                                rh = h - rh * h / th;
                                int l = w - i * tw - tw;
                                draw_tick(l, rh);
                            }
        ...
        {
            Graphics graphics = tabPage2.CreateGraphics();//声明一个Graphics对象  
            Brush fontBrush = SystemBrushes.GradientActiveCaption;
            Font drawFont = new Font("Arial", 16);
            graphics.DrawString("*", drawFont, fontBrush, new PointF(lt, rt));
        }
